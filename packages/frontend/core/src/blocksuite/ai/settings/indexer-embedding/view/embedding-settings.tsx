...
export const EmbeddingSettings: React.FC<EmbeddingSettingsProps> = () => {
  ...
  const ignoredDocNodes = useMemo(
    () => ignoredDocs.edges.map(edge => edge.node),
    [ignoredDocs]
  );
  ...
  const handleIgnoredDocsPageChange = useCallback(
    (offset: number) => {
      embeddingService.embedding.getIgnoredDocs({
        offset,
        after: ignoredDocs.pageInfo.endCursor,
      });
    },
  ...
  return (
    <SettingWrapper title={t['Embedding']()}>
      <SettingRow
        name=""
        desc={t[
      ...
      >
        <Switch checked={embeddingEnabled} onChange={handleEmbeddingToggle} />
      </SettingRow>
      ...
      >
        <Upload fileChange={handleAttachmentUpload}>
          <Button variant="primary">{t['Upload File']()}</Button>
        </Upload>
      </SettingRow>
      ...
      >
        <Button variant="primary" onClick={handleSelectDoc}>
          {t['Select Doc']()}
        </Button>
      </SettingRow>
      ...
      {ignoredDocNodes.length > 0 && (
        <IgnoredDocs
          ...
          totalCount={ignoredDocs.totalCount}
          onPageChange={handleIgnoredDocsPageChange}
        />
      )}
    </SettingWrapper>
  );
