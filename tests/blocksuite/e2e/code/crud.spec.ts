...
test('code block preview mode for html lang', async ({ page }) => {
  await enterPlaygroundRoom(page);
  await initEmptyCodeBlockState(page, { language: 'html' });
  await focusRichText(page);

  const iframe = page.locator('affine-code iframe');

  await type(page, '<div>const a = 10;</div>');

  await assertRichTexts(page, ['<div>const a = 10;</div>']);
  ...
  expect(await iframe.getAttribute('srcdoc')).toEqual(
    expect.stringContaining('<div>const a = 10;</div>')
  );
...
test('change lang when enable preview', async ({ page }) => {
  await enterPlaygroundRoom(page);
  await initEmptyCodeBlockState(page, { language: 'html' });
  await focusRichText(page);
  ...
  // hover and check preview button
  await code.hover({
    position: {
      x: 155,
      y: 65,
